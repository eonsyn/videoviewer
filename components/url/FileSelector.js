...
import { Play, FileIcon, Film, Image as ImageIcon, Loader } from "lucide-react";
    ...
    <div className="w-full mt-3 rounded-2xl border border-white/[0.1] bg-[#0f0f0f] overflow-hidden flex flex-col">
      ...
      <div className="flex flex-col py-2 px-1 sm:px-2 gap-1 overflow-y-auto max-h-[600px]">
        {files.map((file, idx) => {
          ...
          const isVideo =
            file.isVideo === true || isVideoFile(file.filename);

          const isImage =
            file.isImage === true || isImageFile(file.filename);

          const size = formatSize(file.size);

          /*
           * Determine the actual file type.
           * Video has priority if the API explicitly says it is video.
           */
          ...
          const canPlay = fileType === "video" || fileType === "image";
          ...
          return (
            ...
            >
              ...
              <div className="relative w-[100px] sm:w-[120px] aspect-video flex-shrink-0 rounded-lg overflow-hidden bg-[#1a1f2b] flex items-center justify-center">
                ...
                >
                  {isLoading ? (
                    ...
                    <div className="flex items-end gap-[3px] h-4">
                      <div
                        className="w-[3px] bg-white rounded-sm animate-[equalize_0.8s_ease-in-out_infinite]"
                        style={{
                          height: "60%",
                          animationDelay: "0ms",
                        }}
                    ...
                    /* Play icon only for playable media */
                    <Play
                      size={20}
                ...
                <div
                  className={[
                    "absolute bottom-1 right-1 bg-black/80 px-1.5 py-0.5 rounded text-[10px] font-medium text-white leading-tight transition-opacity",
                    isActive || isLoading
                      ? "opacity-0"
              ...
              <div className="hidden sm:flex w-8 h-8 flex-shrink-0 items-center justify-center rounded-full hover:bg-white/10 opacity-0 group-hover:opacity-100 transition-all text-white">
                ...
                >
                  <path d="M12 16.5c.83 0 1.5.67 1.5 1.5s-.67 1.5-1.5 1.5-1.5-.67-1.5-1.5.67-1.5 1.5-1.5zM10.5 12c0 .83.67 1.5 1.5 1.5s1.5-.67 1.5-1.5-.67-1.5-1.5-1.5-1.5 1.5-1.5 1.5zm0-6c0 .83.67 1.5 1.5 1.5s1.5-.67 1.5-1.5-.67-1.5-1.5-1.5-1.5 1.5-1.5 1.5z" />
                </svg>
              </div>
            </button>
          );
