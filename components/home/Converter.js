...
function Converter() {
    ...
    const handleSubmit = async (serverType) => {
      ...
      if (!trimmedUrl.startsWith("https://")) {
        setError("Invalid URL. Please enter a valid link that starts with https://");
        return;
      }
      ...
      try {
        ...
        const data =
          serverType === "server1" ? await server_1(trimmedUrl) : await server_2(trimmedUrl);
        ...
        <div className="w-full md:px-32 rounded-2xl p-6  ">
          ...
          {error && (
            <p className="text-red-500 text-center font-medium bg-red-500/10 border border-red-600 rounded-lg py-2">
              There was an error use Different Url <br/>
              {error}
            </p>
          )}
        </div>
