...
export default function Converter({ token, url }) {
      ...
      try {
        ...
        if (finalStreamUrl && finalStreamUrl.includes("/playlist/") && finalStreamUrl.includes(".m3u8")) {
          try {
            ...
            } else if (videoData.stream) {
              finalStreamUrl = videoData.stream;
            } else if (videoData.download_link) {
              finalStreamUrl = videoData.download_link;
            }
          }
        }
  ...
  // ✅ SAFELY EXTRACT FALLBACK STRING URL
  let fallbackStringUrl = selectedVideo?.fast_stream_url || "";
  if (typeof fallbackStringUrl === "object" && fallbackStringUrl !== null) {
    fallbackStringUrl = Object.values(fallbackStringUrl)[0] || "";
  }
      ...
      {!loading && !error && selectedVideo && (
        <div className="flex flex-col lg:flex-row gap-6 mt-6">
          <div className="flex-1 min-w-0">
            ...
            >
              <CustomPlayer
                src={isHlsError && fallbackStringUrl ? fallbackStringUrl : selectedVideo.stream_url}
                fallbackSrc={fallbackStringUrl}
